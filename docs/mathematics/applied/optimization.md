...
- $c \in \mathbb{R}^n$ — вектор коэффициентов целевой функции
- $A \in \mathbb{R}^{m \times n}$ — матрица ограничений
- $b \in \mathbb{R}^m$ — вектор правых частей
...
**Двойственная задача**: Для прямой задачи $\min c^T x$ при $Ax \geq b, x \geq 0$ двойственная:
$$\begin{align}
\max \quad & b^T y \\
...
- **Слабая двойственность**: $c^T x^* \geq b^T y^*$ для любых допустимых решений
- **Сильная двойственность**: При оптимальности $c^T x^* = b^T y^*$

### Симплекс-метод

**Алгоритм симплекс-метода**:
1. Приведение к канонической форме
2. Поиск начального допустимого базисного решения
3. Проверка критерия оптимальности: $c_j - z_j \leq 0$ для всех $j$
4. Выбор входящей переменной: $\max_j (c_j - z_j)$
5. Выбор выходящей переменной: $\min_i \frac{b_i}{a_{ij}}$ для $a_{ij} > 0$
