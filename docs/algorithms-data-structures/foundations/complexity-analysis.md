...
> **Big O Notation и оценка производительности алгоритмов**
>
...
**Big O notation** описывает верхнюю границу времени выполнения алгоритма в зависимости от размера входных данных.

**Основные принципы:**
- Описывает **наихудший случай** (worst-case scenario)
- Игнорирует константы и младшие слагаемые
- Фокусируется на **росте** времени выполнения при увеличении входных данных
...
4. **Рекурсия с двумя вызовами:** O(2ⁿ)
5. **Рекурсия с мемоизацией:** часто O(n)

### Оптимизация алгоритмов

```go
// ==================== ПАТТЕРН: TRADE-OFF МЕЖДУ ВРЕМЕНЕМ И ПАМЯТЬЮ ====================

// Вариант 1: Быстро, но много памяти
func fastButMemoryHungry(data []int) map[int]bool {
    result := make(map[int]bool, len(data))  // O(n) памяти
    
    for _, val := range data {               // O(n) времени
        result[val] = true                   // O(1) вставка
    }
...
// Вариант 2: Медленно, но экономно с памятью
func slowButMemoryEfficient(data []int, target int) bool {
    for _, val := range data {               // O(n) времени
        if val == target {
            return true                      // O(1) памяти
        }
    }
    return false
...
*Big O - это компас в мире алгоритмов. Он показывает направление, но не точное время прибытия.* 🧭 